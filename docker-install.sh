...
do_install() {
	...
	case "$lsb_dist" in
		...
		ubuntu|debian)
			...
			$sh_c "sleep 3; apt-get update; apt-get install -y -q docker-engine=1.11.1-0~${dist_version}"
			)
			echo_docker_as_nonroot
			exit 0
			;;
		...
		fedora|centos|oraclelinux)
			...
			if [ "$lsb_dist" = "fedora" ] && [ "$dist_version" -ge "22" ]; then
				(
					set -x
					$sh_c 'sleep 3; dnf -y -q install docker-engine-1.11.1'
				)
			else
				(
					set -x
					$sh_c 'sleep 3; yum -y -q install docker-engine-1.11.1'
				)
			fi
			echo_docker_as_nonroot
			exit 0
